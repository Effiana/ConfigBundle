...
use Effiana\ConfigBundle\Tests\IntegrationTestBundle\Entity\CustomSetting;
...
class SettingsControllerIntegrationTest extends IntegrationTestCase {
	...
	public function testModifyAction_noSettings($platform, $config, $requiredExtension) {
		...
		$client->request('GET', $this->url($client, 'effiana_config_settings_modify'));
		$content = $client->getResponse()->getContent();
		$this->assertSame(200, $client->getResponse()->getStatusCode(), $content);
		$this->assertContains('<div class="effiana_config_settings_modify">', $content);
	...
	public function testModifyAction_noChanges($platform, $config, $requiredExtension) {
		...
		$crawler = $client->request('GET', $this->url($client, 'effiana_config_settings_modify'));
		$content = $client->getResponse()->getContent();
		$this->assertSame(200, $client->getResponse()->getStatusCode(), $content);
		$this->assertRegExp('/<form .*method="post" .*class="effiana_config_settings_modify".*>/', $content);
		$this->assertContains('<label for="effiana_config_modifySettings_settings_0_value">name</label>', $content);
		$this->assertContains('<input type="text" id="effiana_config_modifySettings_settings_0_value" name="effiana_config_modifySettings[settings][0][value]" value="value" />', $content);
	...
	public function testModifyAction_changeValue($platform, $config, $requiredExtension) {
		...
		$crawler = $client->request('GET', $this->url($client, 'effiana_config_settings_modify'));
		$form = $crawler->selectButton('apply')->form();
		$this->assertFalse($form->has('effiana_config_modifySettings[settings][0][name]'));
		$this->assertFalse($form->has('effiana_config_modifySettings[settings][0][section]'));
		$client->followRedirects();
		$client->submit($form, [
			'effiana_config_modifySettings[settings][0][value]' => 'new-value',
		]);
		$content = $client->getResponse()->getContent();
		$this->assertContains('<div class="notice">The settings were changed.</div>', $content);
	...
	public function testModifyAction_changeValue_cacheUsage($platform, $config, $requiredExtension, $environment) {
		...
		$client->followRedirects();
		$client->submit($form, [
			'effiana_config_modifySettings[settings][0][value]' => 'new-value1',
		]);
	...
	public function testModifyAction_formInvalid($platform, $config, $requiredExtension) {
		...
		$crawler = $client->request('GET', $this->url($client, 'effiana_config_settings_modify'));
		$form = $crawler->selectButton('apply')->form();
		$form->remove('effiana_config_modifySettings[settings][0][value]');
		$client->followRedirects();
		$client->submit($form);
		$content = $client->getResponse()->getContent();
	...
	public function testModifyAction_properTranslations($platform, $config, $requiredExtension) {
		...
		$this->assertContains('<label for="effiana_config_modifySettings_settings_0_value">setting no. 1</label>', $content);
	...
	public function testModifyAction_sectionOrder_defaultOrder($platform, $config, $requiredExtension) {
		...
		$strPosField1 = strpos($content, '<label for="effiana_config_modifySettings_settings_0_value">name1</label>');
		$strPosField2 = strpos($content, '<label for="effiana_config_modifySettings_settings_1_value">name2</label>');
		$strPosField3 = strpos($content, '<label for="effiana_config_modifySettings_settings_2_value">name3</label>');
	...
	public function testModifyAction_redirectRouteAfterModify($platform, $config, $requiredExtension) {
		...
		$this->assertSame('admin_settings_start', $client->getContainer()->getParameter('effiana_config.redirectRouteAfterModify'));
	...
	public function testModifyAction_customEntity($platform, $config, $requiredExtension, $environment) {
		...
		$crawler = $client->request('GET', $this->url($client, 'effiana_config_settings_modify'));
		$content = $client->getResponse()->getContent();
		$this->assertContains('<textarea id="effiana_config_modifySettings_settings_0_value" name="effiana_config_modifySettings[settings][0][value]">value</textarea>', $content);

		$form = $crawler->selectButton('apply')->form();
		$client->followRedirects();
		$client->submit($form, [
			'effiana_config_modifySettings[settings][0][value]' => $newValue,
		]);
