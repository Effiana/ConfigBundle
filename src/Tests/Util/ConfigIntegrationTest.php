...
use Effiana\ConfigBundle\Tests\IntegrationTestBundle\Util\CustomConfig;
...
class ConfigIntegrationTest extends IntegrationTestCase {
	...
	public function testCacheUsage($platform, $config, $requiredExtension, $environment) {
		...
		$container->get('effiana_config')->all();

		$this->assertTrue($container->get('effiana_config_cache_adapter')->has('name'));
	}
	...
	public function testCustomEntity($platform, $config, $requiredExtension, $environment) {
		...
		$customConfig = $client->getContainer()->get('effiana_config');
	 ...
	 * @expectedExceptionMessage An exception occurred while executing 'INSERT INTO effiana_config_setting
	 ...
	 *
	 * @expectedException \Doctrine\DBAL\Exception\UniqueConstraintViolationException
	 * @expectedExceptionMessage An exception occurred while executing 'INSERT INTO effiana_config_setting_custom
	 * @expectedExceptionMessage Integrity constraint violation: 1062 Duplicate entry 'name1' for key 'PRIMARY'
	 */
